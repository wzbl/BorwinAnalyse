...
namespace GC.Frame.Motion.Privt
{
    public class CNMCLib20
    {
        ...
        /// 回零状态字位定义
        /// </summary>
        public const Int32 BIT_AXHOME_BUSY = 0x00000001;    // bit 0：该位为1，回零中   
        public const Int32 BIT_AXHOME_OK = 0x00000002;      // bit 1：该位为1，回零成功  
        public const Int32 BIT_AXHOME_FAIL = 0x00000004;    // bit 2：该位为1，回零失败  
        public const Int32 BIT_AXHOME_ERR_MV = 0x00000008;  // bit 3：该位为1，回零错误：运动参数出错导致不动 
        public const Int32 BIT_AXHOME_ERR_SWT = 0x00000010; // bit 4：该位为1，回零错误：搜寻过程中开关没触发 
        ...
        // planeIdx:平面类型，0:XY, 1:YZ, 2:ZX,目前只支持XY平面整圆插补
        // pStPos:起点位置点坐标（二维数组,分别表示起点的XY轴的位置）
        // pMidPosArray:中间位置点坐标（二维数组,分别表示中间点的XY轴的位置）
        // pTgPosArray：终点位置坐标（二维数组,分别表示终点的XY轴的位置）
        [DllImport(DLL_PATH, CallingConvention = CallingConvention.StdCall)]
        public static extern Int16 NMC_CrdCirclePPP(HAND crdHandle, Int32 segNo,short planeldx, Int32[] pStPos,Int32[] pMidPosArray, Int32[] pTgPosArray, double endVel, double vel, double synAcc, short lookaheadDis);
       ...
       /// <summary>
        ...
        /// <summary>
        ///  设置伺服OFF, 建议轴静止时执行，如果后面是update指令，需要延时一个周期
        /// </summary>
        /// <param name="axisHandle">轴句柄</param>
        /// <returns></returns>
        [DllImport(DLL_PATH, CallingConvention = CallingConvention.StdCall)]
        ...
        [DllImport(DLL_PATH, CallingConvention = CallingConvention.StdCall)]
        public static extern Int16 NMC_GetErrDesc(Int16 errCode, [MarshalAs(UnmanagedType.LPArray, SizeConst = 512)] byte[] pData);
    ...
    ;
        ...
        public struct TAdvCaptureParamEx
        {
            ...
            short capAxMask;                    // 指捕获的编码器位置源掩码
            short trigSrc;                      // 触发信号类型,见上面的定义。（默认：CAPT_EX_SRC_GPI）
            short trigIndex;                    // 触发源序号。（默认0）
            short trigLev;                      // 触发电平，0：上升，1：下降沿
            short filter;                       // 滤波时间常数,单位:微秒,取值范围[0,255]
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
            short[] reserved;                  // 保留
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 8)]
            int[] reserved2;                  // 保留
        ...
        public struct TPvtMoveHighToLow
        {
            ...
            public short smooth;      // 平滑系数取值[0,50]
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
